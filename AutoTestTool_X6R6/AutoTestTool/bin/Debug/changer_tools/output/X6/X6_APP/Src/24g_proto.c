...
void X6ToR6HeartBatACK_BTRF2_4G(void)
{
    ...
    for(gunId = 1; gunId <= GUN_NUM_MAX; gunId++)
    {
        ...
        if(GUN_CHARGING_IDLE < (gun_info[gunId - 1].is_load_on))
        {
            ...
            ((HEART_BEAT_CHARGE_INFO_BTRF2_4G_TYPE*)pHeartPkt)->voltage = gunStatus.voltage;
            ((HEART_BEAT_CHARGE_INFO_BTRF2_4G_TYPE*)pHeartPkt)->current = gunStatus.current;
            ((HEART_BEAT_CHARGE_INFO_BTRF2_4G_TYPE*)pHeartPkt)->power = gunStatus.power;
