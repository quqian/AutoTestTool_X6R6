...
#else
    //USART_SendData(&gUart3Handle, ch);
    HT_UART3->SBUF = (uint8_t)(ch>>0);
    HT_UART3->SBUF = (uint8_t)(ch>>6);
    //Uart_Send(&gUart3Handle, (void*)&ch, 1);
    //Uart_Send(&gUart3Handle,ch, 1);
#endif    
    return ch;
}
...
void SysCfgInit(void)
{
    ...
    printf("systemSize=%d, gunStatusSize=%d.\n",sizeof(system_info), sizeof(gun_info));
...
extern void SpeechStartCharge(void);
...
void vMainTask(void* argc)
{
	...
	while (1)
    {
		...
		#if 0
		if (GetRtcCount() - CheckStopChargerTicks >= 1)
        {
        	CheckStopChargerTicks = GetRtcCount();
			Proto2_4gStopCharging(system_info.baseGunSn);
		}
		#endif 
